...
def build_dataloaders(dataset_name, batch_size, train_frac=0.8, seed=SEED):
    ...
    train_size = int(train_frac * len(training_data))
...
def _parse_experiment_dir_name(dirname):
    ...
    Returns dict with keys: kind, init_width, lambda_penalty, junctures_mode, label.
    """
    name = str(dirname)
...
def plot_param_count_vs_test_acc(
    ...
    pareto_scope : str
        "global" for one frontier over all points; "per_kind" for separate baseline/plasticity lines.
    ...
    pareto_frontier_kinds : tuple of str
        Which experiment kinds receive a frontier line when pareto_scope="per_kind".
    pareto_linestyle : str or dict
        Line style for frontier(s); dict maps kind -> linestyle.
        ...
        if pareto_scope == "global":
            ...
            for kind in pareto_frontier_kinds:
                if kind not in pareto_source["kind"].values:
                    continue
                ls = (
                    pareto_linestyle[kind]
                    if isinstance(pareto_linestyle, dict)
                    else pareto_linestyle
                )
                handle = _draw_kind_pareto_frontier(
                    ax,
                    pareto_source,
                    kind,
...
# done 5e-06,5e-08
    ...
    #             main(
    #                 f"results_kmnist_g01_r01_global_param_200_neuron_total_nocool_mad_300epochs/run_{i}",
    #                 hidden_sizes,
    #                 lambda_penalty,
    #                 junctures_mode=junctures_mode,
    #                 dataset="kmnist",
    #                 run_mode="plasticity",
    #             )
    # print("All KMNIST experiments completed.")

    # # # Resume hybrid refinement from an existing plasticity run (skips plasticity training):
    # for lambda_penalty in [1e-06]:
    #         set_seed(SEED)
    #         for i in range(1,6):
    ...
    #                 resume_from_plasticity_dir=(
    #                     f"results_g01_r01_global_param_200_neuron_total_nocool_mad_300epochs/run_{i}/plasticity_400_{lambda_penalty}"
    #                 ),
    #                 run_mode="hybrid",
    #             )
...
# )
    ...
    # plot_param_count_vs_test_acc(
    #     save_path="results_g01_r01_global_param_200_neuron_total_nocool_mad_300epochs",
    #     experiments=[
    #        "baseline_20","baseline_100","baseline_200","baseline_300",
    #         "three_phase_baseline_20",
    #         "three_phase_baseline_100","three_phase_baseline_200",
    #         "three_phase_baseline_300",
    #         "plasticity_400_5e-06","plasticity_400_1e-06","plasticity_400_5e-07","plasticity_400_1e-07","plasticity_400_5e-08","plasticity_400_5e-09",
    ...
    #     pareto_frontier_kinds=("baseline", "three_phase", "plasticity", "plasticity_three_phase"),
    #     save_path_out="results_g01_r01_global_param_200_neuron_total_nocool_mad_300epochs.png",
