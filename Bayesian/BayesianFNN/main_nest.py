"""
Bayesian-inspired NeST (Dai, Yin & Jha) on Fashion-MNIST.

Two-hidden-layer sparse Bayesian FNN:
  - Seed: activate ~10% of connections with path connectivity guaranteed.
  - Growth phase: gradient-based connection growth (Policy 1) and neuron
    growth with square-root init + birth strength (Policy 2 / Algorithm 1).
  - Prune phase: iterative connection pruning by weight SNR |μ|/σ (NeST
    magnitude prune with SNR substituted), then retrain.

Backbone matches main2 (LayerNorm + SiLU). No λ param-count penalty.
"""
...
from BayesianFNN import BayesianLinear
...
# ---------------------------------------------------------------------------
# Growth signal collection (mean-field)
# ---------------------------------------------------------------------------
...
def run_nest_experiment(
    ...
    # Final eval on last accepted model (post-prune) — this is the reported model.
    ...
    # Same layout as main2.write_experiment_summary_csv.
    # Parameters = active (masked) count so Pareto size matches effective capacity.
    # Metrics describe the final accepted NeST model (post-prune).
    active_params = model.active_param_count()
    write_experiment_summary_csv(
        output_dir,
...
def main(
    ...
    """
    Run Bayesian NeST on Fashion-MNIST.

    hidden_sizes: seed widths before growth (scaled by seed_scale).
    seed_activate_frac: fraction of connections active at seed (NeST default 0.1).
    """
...
if __name__ == "__main__":
    ...
    for prune_acc_floor in [89.0, 88.5, 88.0, 87.5, 87.0]:
        for i in range(1, 6):
            set_seed(SEED + i)
            main(
                f"results_nest_FashionMnist_FNN/run_{i}",
                hidden_sizes=(300, 100),
                seed_scale=1,  
                seed_activate_frac=0.1,
                reference_acc=89.0,
